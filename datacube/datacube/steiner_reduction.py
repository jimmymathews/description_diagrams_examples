...
from .progress_bar import ProgressBar, ProgressingTask
    ...
    def do_regularization(self, diagram):
        ...
        for point_signature, feature_signature in existing_edges:
            ...
            for i in range(len(permutation)-1):
                s1 = diagram.facet(permutation[0:(i+1)]).signature
                s2 = diagram.facet(permutation[0:(i+2)]).signature
                if graph.has_edge(s2, s1):
                    support = graph.edges[s2, s1]['supports feature inference']
                    graph.edges[s2, s1]['supports feature inference'] = support + [(p, f)]
                else:
                    graph.add_edge(s2, s1)
                    graph.edges[s2, s1]['supports feature inference'] = [(p, f)]
        logger.info(
            'Weights: %s',
